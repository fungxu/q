...
array_reduce(
    [
        ...
        "fapply", "fcall", "fbind",
        "all", "allResolved",
        "iterate", "map", "reduce", "forEach", "buffer",
        "timeout", "delay",
 ...
 */
Q.reduce = reduce;
function reduce(source, callback, basis, maxInFlight, notify) {
    var iterator = resolve(source).iterate();
    var inFlight = 0;
    var done = false;
    var deferred = defer();
    ...
    function next() {
        ...
        .then(function (iteration) {
            if (iteration.done) {
                ...
                ++inFlight;
                if (notify) {
                    fcall(notify, inFlight, maxInFlight).done();
                }
                next();
 ...
 */
Q.map = map;
function map(input, callback, maxInFlight, notify) {
    var inFlight = 0;
    var done = false;
    var pipe = Pipe(input, maxInFlight, notify);
    reduce(pipe.input, function (undefined, value, index) {
        inFlight++;
 ...
 */
Q.forEach = forEach;
function forEach(source, callback, maxInFlight, notify) {
        ...
        semaphore.get().then(function () {
            ...
            .then(function (iteration) {
                ...
                if (iteration.done) {
                    ...
                    return resolve(iteration.value)
                    .then(function (value) {
                        return fcall(callback, value, iteration.index);
                    })
                    .then(function () {
                        if (notify) {
                            fcall(notify, --inFlight, maxInFlight).done();
 ...
 */
Q.buffer = buffer;
function buffer(input, maxInFlight, notify) {
    var pipe = new Pipe(input, maxInFlight, notify);
    var iterator = resolve(pipe.input).iterate();
    function next() {
        return iterator.invoke("next")
        .then(function (iteration) {
            next();
...
function all(input) {
    var output = [];
    return reduce(input, function (undefined, value, index) {
        output[index] = value;
    }, void 0)
    .then(function () {
        return output;
...
Queue.prototype.map = function (callback, maxInFlight, notify) {
    return resolve(this).map(callback, maxInFlight, notify);
};
...
Queue.prototype.reduce = function (callback, basis, maxInFlight) {
    if (arguments.length < 2) {
        ...
        return resolve(this).reduce(callback, basis, maxInFlight);
    }
};
...
Queue.prototype.forEach = function (callback, maxInFlight) {
    return resolve(this).forEach(callback, maxInFlight);
};
...
Queue.prototype.buffer = function (maxInFlight, notify) {
    return resolve(this).buffer(maxInFlight, notify);
};
...
function Pipe(trueInput, maxInFlight, notify) {
    ...
    trueInput = resolve(trueInput).iterate();
